...
export async function getVotedJokeIds(ip) {
  ...
  const result = await client.send(new GetCommand({
    ...
    Key: { PK: `VOTER#${ip}`, SK: 'HISTORY' }
  }));
  const votedIds = result.Item?.votedIds;
  if (!votedIds) return new Set();
...
async function markVoted(ip, jokeId) {
  ...
  await client.send(new UpdateCommand({
    TableName: RATINGS_TABLE,
    Key: { PK: `VOTER#${ip}`, SK: 'HISTORY' },
    UpdateExpression: 'ADD votedIds :id',
    ExpressionAttributeValues: { ':id': new Set([jokeId]) }
  }));
}
...
export async function writeRating({ jokeId, rating, joke, author, mode, dateKey, ip }) {
  if (ip) {
    const voted = await getVotedJokeIds(ip);
    if (voted.has(jokeId)) {
