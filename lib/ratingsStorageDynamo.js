...
// Per-author aggregate stats (one row per distinct `author` value ever rated) —
// used by the dashboard and by the AI-vs-human comparison on /community-jokes.
export async function getAuthorStats() {
  const client = getDynamoClient();
  const authorsResult = await client.send(new QueryCommand({
    TableName: STATS_TABLE,
  ...
  return (authorsResult.Items || []).map(item => ({
...
export async function getDashboardStats() {
  ...
  const [globalItem, authors, topResult, recentResult, uniqueJokesCount] = await Promise.all([
