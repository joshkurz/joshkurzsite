...
import { getDynamoClient, RATINGS_TABLE, STATS_TABLE, PutCommand, GetCommand, QueryCommand, UpdateCommand } from './dynamoClient.js';

const RATE_LIMIT_MINUTES = 60;

// Returns { allowed: true } or { allowed: false }.
// Uses a conditional UpdateItem so the check+record is atomic — no separate read needed.
export async function checkAndRecordRateLimit(ip, jokeId) {
  ...
  try {
    await client.send(new UpdateCommand({
      ...
      Key: { PK: `RATELIMIT#${hashedIp}`, SK: `JOKE#${jokeId}` },
      UpdateExpression: 'SET lastRatedAt = :now',
      ConditionExpression: 'attribute_not_exists(PK) OR lastRatedAt < :cutoff',
