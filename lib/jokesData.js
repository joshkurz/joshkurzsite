...
export { CATEGORIES, CATEGORY_META } from './categories.js'
...
export function getJokeById(id) {
  return getAllJokes().find((joke) => joke.id === id) || null
}

export function getJokesByCategory(category) {
  return getAllJokes().filter((joke) => joke.category === category)
}
