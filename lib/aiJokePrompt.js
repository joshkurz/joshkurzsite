...
async function getRandomWikipediaTopic() {
  try {
    const res = await fetch('https://en.wikipedia.org/api/rest_v1/page/random/summary', {
      headers: { 'User-Agent': 'joshkurz-dadjokes/1.0' },
    });
    if (!res.ok) return null;
    const data = await res.json();
    const title = data.title || '';
    const extract = data.extract ? data.extract.split('.')[0] : '';
...
export async function buildAiJokePrompt() {
  const [inspirationBlock, wikiTopic] = await Promise.all([
    buildInspirationBlock(),
    getRandomWikipediaTopic(),
  ]);
...
- Calendar / clock / "days numbered" jokes

YOUR PROCESS (think this through internally — do not include your reasoning in the output)
1. Start with this randomly assigned subject: **${wikiTopic ? `"${wikiTopic.title}"${wikiTopic.extract ? ` (${wikiTopic.extract})` : ''}` : 'something from everyday life'}**. Find a word, phrase, or concept connected to it that has a funny double meaning, then build the joke from there. You don't have to make it about this topic literally — use it as a creative springboard.
