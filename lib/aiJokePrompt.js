import { buildInspirationBlock, getRecentAiJokeSetups } from './jokeInspiration.mjs';

export const AI_JOKE_PROMPT_VERSION = '3';

export async function buildAiJokePrompt() {
  const [inspirationBlock, recentSetups] = await Promise.all([
    buildInspirationBlock(),
    getRecentAiJokeSetups(),
  ]);
...
${recentSetups.length ? `\nALREADY GENERATED — these jokes exist in the database, do not reproduce them or anything similar:\n${recentSetups.map(s => `- "${s}"`).join('\n')}` : ''}
